...
function DefenseShield() {
  ...
  const handleCopyArgument = async () => {
    ...
    const densityMetric = selected.metrics.find(
      (m) => m.id === "density"
    );
    ...
    if (densityMetric) {
          ...
          selected.metrics
            .filter((m) => {
              const oursBetter = m.higherIsBetter
                ? m.ours > m.them
                : m.ours < m.them;
              return m.id !== "density" && oursBetter;
            })
            .map((m) => m.label)
            .join(", ") || "package and overall livability"
        }.`;
      } else {
      ...
      message = `Boss, comparing ${selected.name} vs our project: we stand out on ${selected.metrics
        .filter((m) => {
          const oursBetter = m.higherIsBetter
            ? m.ours > m.them
            : m.ours < m.them;
        ...
        .map((m) => m.label)
        .join(", ") || "overall value and livability"}.`;
    }
