...
import { loadHandbook } from "../../utils/handbookStorage";
...
function computeStatutoryDeductions(basicSalary, children) {
  ...
  // Very rough SOCSO approximation (tiered but simplified)
...
function IncomeRecognition({ kyc, onIncomeSummaryChange }) {
  ...
  const rows = React.useMemo(() => {
    const hb = loadHandbook(); // Always fetch latest (after settings change)
                    ...
                    <td className="px-3 py-2 text-right">
                      ...
                      {!row.flags.isManualHaircut && (
                        <div className="mt-0.5 text-[10px] text-slate-500">
                          Default {formatPercent(row.meta.defaultHaircutPercent)}
                        </div>
                      )}
                    </td>
                    <td className="px-3 py-2 text-right">
            ...
            <tfoot className="bg-slate-900/80 text-slate-200">
              <tr>
                ...
                <td className="px-3 py-2 text-right text-slate-300">
                  {/* Sum statutory over salary row only for clarity */}
                  {formatCurrency(
