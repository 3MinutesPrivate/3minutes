...
function SimulationMitigation({ incomeSummary, commitmentResult }) {
  const netIncome = commitmentResult?.netIncome || incomeSummary?.totalNetIncome || 0;
              ...
              {bestDebtForSettlement && debtFixSim ? (
                <>
                  <div className="text-[11px] text-slate-400">
                    ...
                    (RM {formatCurrency(bestDebtForSettlement.installment)} /m vs
                    outstanding {formatCurrency(bestDebtForSettlement.balance)}).
                  </div>
                  <div className="mt-2 text-[11px] text-slate-300">
